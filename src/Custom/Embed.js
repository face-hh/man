...
export default class {
	...
	addField(name, value) {
		...
		if (this.description.includes(fieldS)) {
			const strArr = this.description.split(fieldS);
	...
	setColor(color = '#202020') {
		if (color === 'RANDOM') return this.color = '#' + ('000000' + Math.floor(Math.random() * 16777215).toString(16)).slice(-6);
		this.colour = color;
		return this;
	}
